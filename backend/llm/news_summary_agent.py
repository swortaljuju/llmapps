...
SUMMARY_WITH_USER_PREFERENCE_AND_CHUNKED_DATA_PROMPT = """
    ...
    Third, you should write the summary in accordance with the user preferences.
    ...
    {content: "content summary", url: ["https://example.com/1", "https://example.com/2"]}
...
class NewsSummaryOutput(BaseModel):
    ...
    summary : str | None = Field(
        default=None,
        description="""Content summarized from news entries content in the prompt. """)
...
class NewsSummaryWithPreferenceAppliedOutput(BaseModel):
    """
    Generated news summary entry. Each entry is a summary of multiple news entries. The title is generated based on the news entries. 
    The content is optional. If based on the user preference and other factors, the summary should be expanded, then crawl the reference urls 
    and summarize them into a paragraph less than 100 words and put the summary in the content field. If you fail to crawl the reference urls, 
    then set failed_to_expand to True and set the content to None.
    """
    title : str | None = Field(
        default=None,
        description="""Content summarized directly from news entries content in the prompt. """)
    content: str | None = Field(
        default=None,
        description="""Optional summary of optionally crawled content from reference urls.""")
    reference_urls: list[str] | None = Field(
        default=None,
        description="""The summarized news entries' reference URLs. """)
...
__model_with_preference_applied_summary_output = langchain_gemini_client.with_structured_output(NewsSummaryWithPreferenceAppliedListOutput)
...
def summarize_news(news_preference_application_experiment: NewsPreferenceApplicationExperiment, news_chunking_experiment: NewsChunkingExperiment, user_id: int, start_time: datetime, period: NewsSummaryPeriod):
    ...
    if news_chunking_experiment == NewsChunkingExperiment.AGGREGATE_DAILY:
        __chunk_and_summarize_news(news_entry_list, period, news_preference_application_experiment)
    elif news_chunking_experiment == NewsChunkingExperiment.EMBEDDING_CLUSTERING:
        __cluster_and_summarize_news(news_entry_list)    

# NewsChunkingExperiment.AGGREGATE_DAILY
def __chunk_and_summarize_news(news_entry_list: list[NewsEntry], chunking_period_type: NewsSummaryPeriod, news_preference_application_experiment: NewsPreferenceApplicationExperiment) -> list[NewsSummaryWithPreferenceAppliedOutput]:
    ...
    __summarize_rank_expand_news(news_entry_list, news_preference)
    # aggregate to whole period
    __summarize_rank_expand_news([], news_preference)

# For NewsPreferenceApplicationExperiment.WITH_SUMMARIZATION_PROMPT experiment   
def __summarize_rank_expand_news(news_entry_list: list[NewsEntry|NewsSummaryWithPreferenceAppliedOutput], user_preference: str | None)-> list[NewsSummaryWithPreferenceAppliedOutput]:
    pass

# NewsChunkingExperiment.EMBEDDING_CLUSTERING
def __cluster_and_summarize_news(news_entry_list: list[NewsEntry], news_preference_application_experiment: NewsPreferenceApplicationExperiment) -> list[NewsSummaryWithPreferenceAppliedOutput]:
...
def __generate_news_summary_cluster(news_entry_list: list[NewsEntry]) -> list[NewsSummaryOutput]:
...
def _expand_news_if_necessary(news_summary_list:  list[NewsSummaryWithPreferenceAppliedOutput]):
    ...
    for news_summary in news_summary_list:
        if expanded_news_count >= MAX_NEWS_SUMMARY_TO_EXPAND:
            break
        if news_summary.should_expand and news_summary.failed_to_expand:
